...
public class WalletFormController {
    ...
    @FXML
    private TextField transactionFeeField;
    ...
    private void initialize() {
        walletTypeComboBox
                .getItems()
                .setAll(WalletType.values());
    }

    /**
     * Nhận Wallet cần sửa từ WalletController
     * và đổ dữ liệu cũ lên form.
     */
    public void setEditingWallet(Wallet wallet) {

        this.editingWallet = wallet;

        walletNameField.setText(
                wallet.getName()
        );

        walletTypeComboBox.setValue(
                wallet.getType()
        );
        ...
        if (wallet instanceof BankAccount bankAccount) {
            ...
            extraFee =
                    bankAccount.getTransactionFee();

        } else if (wallet instanceof EWallet eWallet) {

            extraFee =
                    eWallet.getFeePercent();
        }

        transactionFeeField.setText(
                String.valueOf(extraFee)
        );
    }
    ...
    private void handleSave() {
        ...
        try {
             ...
             */
                ...
                manager.updateWallet(
                        editingWallet,
                        updatedWallet
                );

                showSuccess(
                        "Đã cập nhật ví thành công."
                );
            }
